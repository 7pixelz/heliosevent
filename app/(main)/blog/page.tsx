              ...
              <Link key={post.id} href={`/blog/${post.slug}`} style={{ textDecoration: 'none' }}>
                <article className="blog-card">
                  <div style={{ height: '200px', background: '#e5e7eb', overflow: 'hidden', position: 'relative' }}>
                    {post.coverImageUrl
                      ? <Image src={post.coverImageUrl} alt={post.title} width={340} height={200} quality={70} unoptimized style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
