...
// Phone number assembled at click-time (not present as static text in the
// page HTML) so scrapers scanning raw page source for wa.me links can't
// harvest it for spam/video-call bots.
const PARTS = ['91', '740', '103', '0000'];

function openWhatsApp() {
  const phone = PARTS.join('');
  const text = encodeURIComponent('Hi, Website Enquiry');
  window.open(`https://wa.me/${phone}?text=${text}`, '_blank', 'noopener,noreferrer');
}

export default function WhatsAppSocialLink() {
  return (
    <a className="soc" href="#" onClick={e => { e.preventDefault(); openWhatsApp(); }} aria-label="WhatsApp">
