  ...
  return (
    <section className="services" id="services">
      ...
      <div className="svc-grid">
        {main.map((svc, i) => (
          <Link key={svc.id} href={svc.name.toLowerCase().includes('wedding') ? 'https://www.nakshatrawedding.com/' : `/${svc.slug}`} target={svc.name.toLowerCase().includes('wedding') ? '_blank' : undefined} rel={svc.name.toLowerCase().includes('wedding') ? 'noopener noreferrer nofollow' : undefined} style={{ textDecoration: 'none' }}>
